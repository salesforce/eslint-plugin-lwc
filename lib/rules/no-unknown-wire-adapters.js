        ...
        return {
            Decorator(node) {
                ...
                if (!isKnownAdapter) {
                    ...
                    let message;
                    switch (adapterDeclarationNode.parent.type) {
                        case 'ImportSpecifier':
                            message = `"${importedAdapterIdentifier}" from "${adapterModule}" is not a known adapter.`;
                            break;

                        case 'ImportDefaultSpecifier':
                        case 'ImportNamespaceSpecifier':
                            message = `"${importedAdapterIdentifier}" from "${adapterModule}" (imported as "${localAdapterIdentifier}") is not a known adapter.`;
                            break;
                    }
