 ...
 */
function reachableDuringSSRPartial(context) {
    let moduleInfo;
    let insideLWC = false;
    ...
    const withinLWCVisitors = {
        Program: (node) => {
            ...
            globalReferences = scope.through;
        },
    ...
    return {
        ...
        isGlobal: (name) => !!globalReferences.find((r) => r.identifier.name === name),
    };
}
...
module.exports.noReferenceDuringSSR = function noReferenceDuringSSR(
    ...
    const {
        ...
        isGlobal,
    } = reachableDuringSSRPartial(context);
