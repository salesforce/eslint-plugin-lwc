...
module.exports.noUnguardedAsyncSSR = function noUnguardedAsyncSSR(context) {
    const {
        withinLWCVisitors,
        isInsideReachableMethod,
        isInsideReachableFunction,
    ...
    const asyncOperations = new Set(['fetch', 'then', 'catch', 'finally']);
    const eventOperations = new Set(['addEventListener', 'removeEventListener', 'dispatchEvent']);

    function checkAsyncOperation(node) {
        if (
            asyncOperations.has(node.callee.name) ||
            (node.callee.property && asyncOperations.has(node.callee.property.name))
        ) {
            context.report({
                node,
                messageId: 'unguardedAsyncOperation',
                data: {
    ...
    function checkEventOperation(node) {
        if (
            eventOperations.has(node.callee.name) ||
            (node.callee.property && eventOperations.has(node.callee.property.name))
        ) {
            context.report({
                node,
                messageId: 'unguardedEventOperation',
                data: {
                    name: node.callee.name || node.callee.property.name,
                },
            });
    ...
    return {
        ...
        CallExpression: (node) => {
            ...
            checkAsyncOperation(node);
            checkEventOperation(node);
        },
        NewExpression: (node) => {
            if (
                (!isInsideReachableMethod() &&
