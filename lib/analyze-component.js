...
function getClassDeclaration(root) {
    // TODO: iterate over method names to look for renderedCallback, connectedCallback, etc
...
function getMethodInterdependencies(classDecl) {
    ...
    for (const methodDefNode of classDecl.body.body) {
        ...
        walk(methodDefNode.value.body, (node) => {
            if (
...
function analyze(root) {
    if (componentCache.has(root)) {
        return componentCache.get(root);
    }
...
const cachedModuleRecords = new Map();
module.exports.analyzeCached = function analyzeCached(root) {
    if (cachedModuleRecords.has(root)) {
        return cachedModuleRecords.get(root);
    } else {
        const record = analyze(root);
        cachedModuleRecords.set(root, record);
        return record;
    }
};
